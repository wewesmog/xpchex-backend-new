...
#Models
from app.models.review_analysis_models import MainState, ReviewAnalysisRequest, Opportunities, Error
...
async def opportunities_analysis_node(state: Dict[str, Any]) -> Dict[str, Any]:
    """Analyze the opportunities of the review."""
    ...
    try:
        ...
        user_prompt = get_opportunities_analysis_prompt(
        ...
        messages = [
            {"role": "system", "content": "You are an opportunities analysis agent. Return response in JSON format."},
            {"role": "user", "content": user_prompt}
        ]
        
        opportunities = call_llm_api(
            messages=messages,
            temperature=0.7,
        ...
        # Update state
        current_state.review_analysis.opportunities = opportunities
        current_state.node_history.append({
            "node_name": "opportunities_analysis_node",
            "response": opportunities.model_dump()  # Convert to dict for serialization
        })
        ...
        error_msg = f"Error in opportunities analysis node: {str(e)}"
        logger.error(error_msg)
        
        # Create Error object instead of appending to list
        current_state.error = Error(
            agent="opportunities_analysis_agent",
