...
#Models
from app.models.review_analysis_models import MainState, ReviewAnalysisRequest, SentimentAnalysis, Error
...
async def sentiment_analysis_node(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        messages = [
            {"role": "system", "content": "You are a sentiment analysis agent. Return response in JSON format."},
            {"role": "user", "content": user_prompt}
        ]
        
        sentiment = call_llm_api(
            messages=messages,
            temperature=0.7,
        ...
        # Update state
        current_state.review_analysis.sentiment = sentiment
        current_state.review_analysis.content = review_content  # Add review content to analysis
        current_state.node_history.append({
            "node_name": "sentiment_analysis_node",
            "response": sentiment.model_dump()  # Convert to dict for serialization
        })
        ...
        error_msg = f"Error in sentiment analysis node: {str(e)}"
